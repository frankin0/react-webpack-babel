...
export default class Canvas extends React.Component{
    ...
    handleMouseMove(e){
        ...
        const {q, r, s} = this.cubeRound(this.pixelToHex(this.Point(offsetX, offsetY))); //console.log(q, r, s);
        const {x, y} = this.hexToPixel(this.Hex(q, r, s)); 
        //this.drawHex(this.canvasInteraction, this.Point(x,y), "green", 2); //controllare queste right
    ...
    addObstacles(){
        ...
        if(obstacles.length <= 0){
            console.log("oprima", obstacles);

            if(!obstacles.includes(JSON.stringify(this.state.currentHex))){ //aggiungo
                //obstacles = [].concat(obstacles, JSON.stringify(this.state.currentHex));
                obstacles = [].concat(obstacles, JSON.stringify(this.state.currentHex));
            }else{
                obstacles.map((l, i) => {   //rimuovo
